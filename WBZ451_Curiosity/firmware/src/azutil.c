...
#include "azutil.h"
#include "nmdrv.h"
//#include "config/default/peripheral/sercom/spi_slave/dti.h"
...
static char SW2_lastState = 0;
static char SW2_currentState = 0;
...
static char pnp_property_topic_buffer[256];
...
static const az_span event_name_button_sw2_span   = AZ_SPAN_LITERAL_FROM_STR("USR-BTN");
static const az_span event_name_press_count_span  = AZ_SPAN_LITERAL_FROM_STR("press_count");

// LED Properties
static const az_span led_blue_property_name_span   = AZ_SPAN_LITERAL_FROM_STR("rgb_led_blue");
static const az_span led_green_property_name_span  = AZ_SPAN_LITERAL_FROM_STR("rgb_led_green");
static const az_span led_user_property_name_span = AZ_SPAN_LITERAL_FROM_STR("led_user");
static const az_span led_red_property_name_span    = AZ_SPAN_LITERAL_FROM_STR("rgb_led_red");
...
void init_twin_data(twin_properties_t* twin_properties)
{
    ...
    twin_properties->desired_led_user   = LED_TWIN_NO_CHANGE;
    twin_properties->desired_led_blue   = LED_TWIN_NO_CHANGE;
    twin_properties->desired_led_green  = LED_TWIN_NO_CHANGE;
    twin_properties->desired_led_red    = LED_TWIN_NO_CHANGE;
...
*/
    ...
    {
        RETURN_ERR_IF_FAILED(append_json_property_double(&jw, telemetry_name_temperature_span, temperature));
    }
...
void check_button_status(void)
{
    ...
    // Read SW2 button input; compare last state vs. current state
    SW2_lastState = SW2_currentState;
    SW2_currentState = SW2_Get();
    if (SW2_lastState != SW2_currentState)
    {
        if (SW2_currentState == 0) // Falling edge
        {
            APP_SW2_Handler();
        }
    }
    ...
    if (sw2_pressed)
    {
        debug_printInfo("AZURE: Button SW2 Count %lu", button_press_data.sw2_press_count);
...
*/
    ...
    RETURN_ERR_WITH_MESSAGE_IF_FAILED(
        build_sensor_telemetry_message(&telemetry_payload_span, temp),
    ...
    twin_properties_t* twin_properties)
{
    ...
    if (twin_properties->flag.user_led_found == 1 && twin_properties->desired_led_user != LED_TWIN_NO_CHANGE)
    {
        if (twin_properties->desired_led_user == LED_TWIN_ON)
        {
            LED_SetUser(LED_STATE_HOLD);
        }
        else if (twin_properties->desired_led_user == LED_TWIN_OFF)
        {
            LED_SetUser(LED_STATE_OFF);
        }
        else if (twin_properties->desired_led_user == LED_TWIN_BLINK)
        {
            LED_SetUser(LED_STATE_BLINK_FAST);
        }
    }

    if (twin_properties->flag.blue_led_found == 1 && twin_properties->desired_led_blue != LED_TWIN_NO_CHANGE)
    {
        if (twin_properties->desired_led_blue == 100)
        {
            BLUE_LED_Set();            
        }
        else if (twin_properties->desired_led_blue == 0)
        {
            BLUE_LED_Clear();
        }
    }
   
    if (twin_properties->flag.green_led_found == 1 && twin_properties->desired_led_green != LED_TWIN_NO_CHANGE)
    {
        if (twin_properties->desired_led_green == 100)
        {
            GREEN_LED_Set();            
        }
        else if (twin_properties->desired_led_green == 0)
        {
            GREEN_LED_Clear();
        }
    }
    
    if (twin_properties->flag.red_led_found == 1 && twin_properties->desired_led_red != LED_TWIN_NO_CHANGE)
    {
        if (twin_properties->desired_led_red == 100)
        {
            RED_LED_Set();            
        }
        else if (twin_properties->desired_led_red == 0)
        {
            RED_LED_Clear();
        }
    }
    ...
    uint16_t* out_response_status)
{
    ...
    *out_response_status = AZ_IOT_STATUS_SERVER_ERROR;
    ...
    else if (spanSize > 100)//SERCOM0_USART_WRITE_BUFFER_SIZE)
    {
    ...
    else
    {
        ...
        debug_disable(true);
        /****LUC*****/
        //When receiving a Azure string message
        void cloud2ble(uint8_t * message);
        cloud2ble(messageString);
        /****LUC*****/
    ...
    twin_properties_t* twin_properties)
{
        ...
        &component_name_span)))
    {
        ...
        else if (az_json_token_is_text_equal(&jr.token, led_user_property_name_span))
        {
            ...
            twin_properties->flag.user_led_found = 1;
        }
        else if (az_json_token_is_text_equal(&jr.token, led_green_property_name_span))
        {
            // found writable property to control Green LED
            RETURN_ERR_IF_FAILED(az_json_reader_next_token(&jr));
            RETURN_ERR_IF_FAILED(az_json_token_get_int32(&jr.token,
                                                         &twin_properties->desired_led_green));
            twin_properties->flag.green_led_found = 1;
        }
        else if (az_json_token_is_text_equal(&jr.token, led_blue_property_name_span))
        {
            // found writable property to control Blue LED
            RETURN_ERR_IF_FAILED(az_json_reader_next_token(&jr));
            RETURN_ERR_IF_FAILED(az_json_token_get_int32(&jr.token,
                                                         &twin_properties->desired_led_blue));
            twin_properties->flag.blue_led_found = 1;
        }
        else if (az_json_token_is_text_equal(&jr.token, led_red_property_name_span))
        {
            // found writable property to control Red LED
            RETURN_ERR_IF_FAILED(az_json_reader_next_token(&jr));
            RETURN_ERR_IF_FAILED(az_json_token_get_int32(&jr.token,
                                                         &twin_properties->desired_led_red));
            twin_properties->flag.red_led_found = 1;
        }
...
#endif
        {
            debug_printError("AZURE: Unable to add property for User LED, return code 0x%08x", rc);
            return rc;
        }
    }
    else if (twin_properties->flag.is_initial_get)
...
bool process_sendString_command(char* data)
{
    az_result      rc = AZ_OK;
    az_json_writer jw;
    char           telemetry_name_buffer[128];
    az_span        telemetry_name_span;
    //az_span        telemetry_payload_span;
    az_span        identifier_span;

            char firmwareString[30] = {0}; // 8bit + 8bit + 8bit + 16bit + 3 dots
//
//        nm_get_firmware_full_info(&fwInfo);
//
//        sprintf(firmwareString, "%u.%u.%u.%u", 
...
//    
    ...
    sprintf(telemetry_name_buffer, "ipAddress");
    telemetry_name_span = az_span_create_from_str((char*)&telemetry_name_buffer);
    sprintf(firmwareString,"%s",data);
    ...
    if (az_result_failed(
                rc = append_json_property_string(
                    ...
                    telemetry_name_span,
                    az_span_create_from_str((char*)&firmwareString))))
//                    telemetry_name_span,//ip_address_property_name_span,
//                    az_span_create_from_str((char*)data))))
        {
            debug_printError("AZURE: Unable to add property for IP Address, return code  0x%08x", rc);
            return rc;
        }
...
//        // A signed 4-byte integer
//        int32_t data_i = strtoll(data, 0, 16);
//        sprintf(telemetry_name_buffer, "rcvMsg");
//        telemetry_name_span = az_span_create_from_str((char*)telemetry_name_buffer);
//        append_json_property_string(&jw, telemetry_name_span, az_span_create_from_str(data));
//    }
//
//    end_json_object(&jw);
//    telemetry_payload_span = az_json_writer_get_bytes_used_in_destination(&jw);
